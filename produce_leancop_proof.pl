...
preprocess_proof([para(ParaOrig-_)|Xs], Counter, Ys):-
    ParaOrig = [Lit, Pos, From, To, Cla, Dir],

    ( Lit=(_=_) -> 
      para_descent(Pos, Lit, From, To, Cla,  Dir, Counter, Steps)
    ; congruence_axiom(Lit, [NegLit|Cla1]),
      Ext1 = [Lit, [NegLit|Cla1]],
      Steps = [ext(Ext1-Ext1, Counter)|Steps0],
      Pos = [P|Pos2],
      Cla1 = [_|Cla2],
      func_subst_substeps(Cla2, P, Pos2, From, To, Cla, Dir, 1, Counter, Steps0)
    ),
...
para_descent([1|Pos], Lit, From ,To, Cla, Dir, Counter, Steps):-
    !,
    %% Lit = (A=B),
    ...
    para_descent([0|Pos], Lit2, From, To, Cla, Dir, Counter, StepsRest),
    append(StepsRest,[StepR], Steps2).
    

para_descent([0], Lit, From, To, Cla, Dir, Counter, Steps):-
    !,
    Lit = (_A=B),
...
para_descent([0, P|Pos], Lit, From, To, Cla, Dir, Counter, Steps):-
    !,
    Lit = (A=B),
    ...
    func_subst_substeps(Cla2, P, Pos, From, To, Cla, Dir, 0, Counter, Steps3).

%% func_subst_substeps(Children, P, Pos, From, To, Cla, Dir, PosPrefix, Counter, Steps)
func_subst_substeps([], _, _, _, _, _, _, _, _, []).
func_subst_substeps([Lit|Children], 0, Pos, From, To, Cla, Dir, PosPrefix, Counter, Steps):-
    !,
    para_descent([PosPrefix|Pos], Lit, From, To, Cla, Dir, Counter, Steps0),
    append(Steps0, Steps1, Steps),
    func_subst_substeps(Children, -1, Pos, From, To, Cla, Dir, PosPrefix, Counter, Steps1).
func_subst_substeps([Lit|Children], P, Pos, From, To, Cla, Dir, PosPrefix, Counter, [S|Steps]):-
    P =\= 0,
    P1 is P-1,
    reflexivity_axiom([NegLitR|ClaR]),
    ExtR = [Lit, [NegLitR|ClaR]],
    S = ext(ExtR-ExtR, Counter),
    func_subst_substeps(Children, P1, Pos, From, To, Cla, Dir, PosPrefix, Counter, Steps).
